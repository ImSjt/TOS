...
static void stride_init(struct run_queue *rq) {
     list_init(&(rq->d_rq.run_list));
     rq->d_rq.task_num = 0;
     rq->d_rq.max_time_slice = 5;
}

static void stride_enqueue(struct run_queue *rq, struct task_struct *task) {
     assert(list_empty(&(task->run_link)));
     list_add_before(&(rq->d_rq.run_list), &(task->run_link));

     if (task->time_slice == 0 || task->time_slice > rq->d_rq.max_time_slice) {
          task->time_slice = rq->d_rq.max_time_slice;
     }
     task->rq = rq;
     rq->d_rq.task_num++;
}
...
static struct task_struct *stride_pick_next(struct run_queue *rq) {
     list_entry_t *le = list_next(&(rq->d_rq.run_list));

     if (le == &rq->d_rq.run_list)
          return NULL;
